...
void lcd_initLcd(void)
{
    ...
    PORTC_PCR8 |= PORT_PCR_MUX( 0x01U );
    PORTC_PCR9 |= PORT_PCR_MUX( 0x01U );


    GPIOC_PDDR |= GPIO_PDDR_PDD( 0x3FFu );
...
void lcd_write2Lcd(unsigned char ucBuffer,  unsigned char cDataType)
{
    ...
    if(LCD_RS_CMD == cDataType)
    	...
    	GPIOC_PCOR |= GPIO_PCOR_PTCO( 0x100u );
    else
        //will send data
        //GPIO_HAL_WritePinOutput(LCD_GPIO_BASE_PNT, LCD_RS_PIN, LCD_RS_DATA);
    	GPIOC_PSOR |= GPIO_PSOR_PTSO( 0x100u );


    //write in the LCD bus
    GPIOC_PCOR |= GPIO_PCOR_PTCO( 0xFFu );
    GPIOC_PSOR |= GPIO_PSOR_PTSO(ucBuffer & (1u << 0u));
    GPIOC_PSOR |= GPIO_PSOR_PTSO((ucBuffer & (1u << 1u)));
    GPIOC_PSOR |= GPIO_PSOR_PTSO(ucBuffer & (1u << 2u));
    GPIOC_PSOR |= GPIO_PSOR_PTSO(ucBuffer & (1u << 3u));
    GPIOC_PSOR |= GPIO_PSOR_PTSO(ucBuffer & (1u << 4u));
    GPIOC_PSOR |= GPIO_PSOR_PTSO(ucBuffer & (1u << 5u));
    GPIOC_PSOR |= GPIO_PSOR_PTSO(ucBuffer & (1u << 6u));
    GPIOC_PSOR |= GPIO_PSOR_PTSO(ucBuffer & (1u << 7u));



    /* enable, delay, disable LCD */
    /* this generates a pulse in the enable pin */
    GPIOC_PSOR |= GPIO_PSOR_PTSO( 0x200u );
    util_genDelay1ms();
    GPIOC_PCOR |= GPIO_PCOR_PTCO( 0x200u );
    util_genDelay1ms();
    util_genDelay1ms();
}
...
void lcd_dummyText(void)
{
    ...
    // send string
    lcd_writeString("Daniel 116528");
    // set the cursor line 1, column 0
    lcd_setCursor(1,0);
    lcd_writeString("Rafael 123144");
